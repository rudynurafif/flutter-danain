...
class NotifLenderBloc extends DisposeCallbackBaseBloc {
  ...
  factory NotifLenderBloc(
    ...
    void getPesan(int status) async {
      ...
      try {
        final response = await apiService.getNotifikasiLender(token, param);
        final List<dynamic> data = response['data_pesan']['data'];
        pesan.addAll(data);
    ...
    void getNotif(int status) async {
      ...
      try {
        final response = await apiService.getNotifikasiLender(token, param);
        final List<dynamic> data = response['data_transaksi']['data'];
        notif.addAll(data);
    ...
    void readNotif(int idNotif) async {
      ...
      final token = event.orNull()!.userAndToken!.token;
      try {
        final response = await apiService.readNotifLender(token, idNotif);
