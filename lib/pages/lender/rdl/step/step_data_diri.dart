...
class StepDataPribadiRDL extends StatefulWidget {
  final RegisRdlBloc rdlBloc;
  const StepDataPribadiRDL({super.key, required this.rdlBloc});

  @override
  State<StepDataPribadiRDL> createState() => _StepDataPribadiRDLState();
}

class _StepDataPribadiRDLState extends State<StepDataPribadiRDL> {
  ...
  @override
  void initState() {
    if (widget.rdlBloc.nameValue.hasValue) {
      _nameController.text = widget.rdlBloc.nameValue.value;
    }

    if (widget.rdlBloc.noKtpValue.hasValue) {
      _noKtpController.text = widget.rdlBloc.noKtpValue.value;
    }

    if (widget.rdlBloc.tempatLahirValue.hasValue) {
      _tempatLahirController.text = widget.rdlBloc.tempatLahirValue.value;
    }
    if (widget.rdlBloc.namaIbuValue.hasValue) {
      namaIbuController.text = widget.rdlBloc.namaIbuValue.value!;
    }
    if (widget.rdlBloc.npwpValue.hasValue) {
      npwpController.text = widget.rdlBloc.npwpValue.value!;
      setState(() {
        npwpString = widget.rdlBloc.npwpValue.value!;
      });
    }

    if (widget.rdlBloc.tanggalLahirValue.hasValue) {
      final val = widget.rdlBloc.tanggalLahirValue.value;
      setState(() {
        tglLahir = val;
      });
    ...
    setState(() {
      gender = widget.rdlBloc.genderValue.hasValue
          ? widget.rdlBloc.genderValue.value
          : '';
    });
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                formGender(context),
                const SizedBox(height: 16),
                statusPerkawinanForm(widget.rdlBloc),
                const SizedBox(height: 16),
                agamaForm(widget.rdlBloc),
                const SizedBox(height: 16),
                pendidikanForm(widget.rdlBloc),
                const SizedBox(height: 16),
                namaIbuForm(widget.rdlBloc),
                const SizedBox(height: 16),
                pekerjaanForm(widget.rdlBloc),
                const SizedBox(height: 16),
                npwpForm(widget.rdlBloc),
                const SizedBox(height: 16),
                penghasilanForm(widget.rdlBloc),
                const SizedBox(height: 16),
                sumberPenghasilanForm(widget.rdlBloc),
                const SizedBox(height: 16),
                tujuanForm(widget.rdlBloc),
                const SizedBox(height: 16),
                buttonNext(widget.rdlBloc),
                const SizedBox(height: 24),
              ],
            ),
          ),
  ...
  Widget formNameKtp() {
    return StreamBuilder<String?>(
        stream: widget.rdlBloc.nameErrorStream,
                ...
                child: Stack(
                  ...
                  children: [
                    TextFormField(
                      ...
                      onChanged: (value) => widget.rdlBloc.nameChange(value),
                    ),
                    if (snapshot.hasData)
                      Padding(
  ...
  Widget formKtp() {
    return StreamBuilder<String?>(
      stream: widget.rdlBloc.ktpErrorStream,
      builder: (context, snapshot) {
        return Column(
          mainAxisAlignment: MainAxisAlignment.start,
          crossAxisAlignment: CrossAxisAlignment.start,
              ...
              child: Stack(
                ...
                children: [
                  TextFormField(
                    ...
                    onChanged: (value) => widget.rdlBloc.ktpChange(value),
                  ),
                  if (snapshot.hasData)
                    Padding(
                      padding: const EdgeInsets.only(top: 68),
  ...
  Widget tempatLahirForm() {
    return StreamBuilder<String?>(
      stream: widget.rdlBloc.tempatLahirErrorStream,
      builder: (context, snapshot) {
        return Column(
          mainAxisAlignment: MainAxisAlignment.start,
          crossAxisAlignment: CrossAxisAlignment.start,
              ...
              child: Stack(
                ...
                children: [
                  TextFormField(
                    ...
                    onChanged: (value) =>
                        widget.rdlBloc.tempatLahirChange(value),
                  ),
                  if (snapshot.hasData)
                    Padding(
                      padding: const EdgeInsets.only(top: 68),
  ...
  Widget formTglLahir(BuildContext context) {
    return StreamBuilder<String?>(
      stream: widget.rdlBloc.tanggalLahirErrorStream,
      builder: ((context, snapshot) {
        return Column(
          mainAxisAlignment: MainAxisAlignment.start,
          crossAxisAlignment: CrossAxisAlignment.start,
        ...
        children: [
          ...
          Padding(
            ...
            child: ButtonNormal(
              btntext: 'Pilih',
              action: () {
                widget.rdlBloc.tanggalLahirChange(tglLahir);
  ...
  Widget formGender(BuildContext context) {
    return StreamBuilder<String?>(
      stream: widget.rdlBloc.genderErrorStream,
      builder: (context, snapshot) {
  ...
  Widget radioContent(String title, String value, bool hasError) {
    return StreamBuilder<String>(
      stream: widget.rdlBloc.genderStream,
      builder: (context, snapshot) {
  ...
  Widget statusPerkawinanForm(RegisRdlBloc bloc) {
  ...
  Widget agamaForm(RegisRdlBloc bloc) {
    return StreamBuilder<Map<String, dynamic>>(
  ...
  Widget pendidikanForm(RegisRdlBloc bloc) {
    return StreamBuilder<Map<String, dynamic>>(
  ...
  Widget namaIbuForm(RegisRdlBloc bloc) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.start,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget pekerjaanForm(RegisRdlBloc bloc) {
  ...
  Widget npwpForm(RegisRdlBloc bloc) {
  ...
  Widget penghasilanForm(RegisRdlBloc bloc) {
    return StreamBuilder<Map<String, dynamic>>(
  ...
  Widget sumberPenghasilanForm(RegisRdlBloc bloc) {
    return StreamBuilder<Map<String, dynamic>>(
  ...
  Widget tujuanForm(RegisRdlBloc bloc) {
    return StreamBuilder<Map<String, dynamic>>(
