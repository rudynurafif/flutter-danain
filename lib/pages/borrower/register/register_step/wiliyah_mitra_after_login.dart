...
class WilayahMitraAfterLogin extends StatefulWidget {
  final RegisterBloc regisBloc;
  const WilayahMitraAfterLogin({super.key, required this.regisBloc});

  @override
  State<WilayahMitraAfterLogin> createState() => _WilayahMitraAfterLoginState();
}

class _WilayahMitraAfterLoginState extends State<WilayahMitraAfterLogin> {
  ...
  void initState() {
    super.initState();
    // Note: You can't directly use `ModalRoute.of(context)` in initState
    // Instead, use a Future.delayed to execute it after the widget has been fully built.
    Future.delayed(Duration.zero, () {
      final dynamic argument = ModalRoute.of(context)!.settings.arguments;
      print('arguments provinsi ${argument.text}');
