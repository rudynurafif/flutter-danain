...
class CasePrivyBorrowerBloc {
  ...
  final casePrivyMessage = BehaviorSubject<PrivyCaseBorrowerMessage>();
  final fileLengthController = BehaviorSubject<List<String>?>();
  final uploadController = BehaviorSubject<List<String>?>();
  ...
  Stream<PrivyCaseBorrowerMessage> get casePrivyMessageStream =>
      casePrivyMessage.stream;

  void initGetCase(String caseCode, List<dynamic> list) {
    print('case privy');
    uploadController.add(list.map((item) => item.toString()).toList());
    print(list);
    ...
    List<String>? typeList = uploadController.valueOrNull;
  ...
  Stream<bool> get isValidButton => Rx.combineLatest2(
    ...
    fileLengthStream,
      ...
      List<String>? mustUploadList = uploadController.value;
  ...
  void postPrivy(int user) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        casePrivyMessage.sink.add(PrivyCaseBorrowerSuccess());
