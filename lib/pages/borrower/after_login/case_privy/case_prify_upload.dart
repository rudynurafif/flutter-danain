...
class CasePrivy extends StatefulWidget {
  static const routeName = '/case_privy';
  final String? caseCode;
  const CasePrivy({super.key, this.caseCode});

  @override
  State<CasePrivy> createState() => _CasePrivyState();
}

class _CasePrivyState extends State<CasePrivy>
    with DisposeBagMixin, DidChangeDependenciesStream {
  final caseBloc = CasePrivyBloc();
  List<String> mustUploadData = [];
  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    final args = ModalRoute.of(context)?.settings.arguments as CasePrivy?;
    if (args != null) {
      caseBloc.initGetCase(args.caseCode!);
    }
  }
  ...
  Stream<void> handleMessage(message) async* {
    if (message is PrivyCaseSuccess) {
  ...
  Widget bodyContent(BuildContext context, Map<String, dynamic> privyCase) {
    List<String> mustUpload = privyCase['mustUpload'];
